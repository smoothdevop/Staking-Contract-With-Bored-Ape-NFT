...
contract Staking {
  ...
  function withdrawStakedToken() public {
    ...
    require(
      staked.timeDeposited > staked.timeDeposited + 30 days,
      "time never reach"
    );
    uint256 stakedamount = staked.tokenAmount;
    uint256 percentageAmount = (stakedamount * 10) / 100;
    uint256 newAmount = stakedamount + percentageAmount;
